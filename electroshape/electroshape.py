...
def electroshape_vector_5d(mol: Chem.Mol) -> np.ndarray:
    ...
    charges = np.array([
        ...
        if mol.GetAtomWithIdx(i).HasProp("ESPCharge") else 0.0 
        for i in range(N)
    ])
...
def process_record(task: Tuple[str, str, str]) -> Optional[dict]:
    molblock, charge_method, unique_id = task
    try:
